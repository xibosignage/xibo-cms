...
<form class="XiboForm form-horizontal" id="<?php echo Theme::Get('form_id'); ?>" method="post" action="<?php echo Theme::Get('form_action'); ?>">
    ...
    <?php foreach(Theme::Get('fields') as $field) { ?>
        <div class="form-group">
            <label class="col-sm-2 control-label" for="<?php echo $field['formfieldid'] ?>" title="<?php echo $field['heading'] ?>"><?php echo $field['heading'] ?></label>
            <div class="col-sm-10">
                <input class="form-control" name="<?php echo $field['formfieldid'] ?>" type="number" id="<?php echo $field['formfieldid'] ?>" value="<?php echo $field['auto_column_number'] ?>" />
            </div>
        </div>
    <?php } ?>
</form>
