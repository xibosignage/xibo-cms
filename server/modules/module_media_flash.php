...
class media_flash 
{
 	...
 	public function AsXml()
	{
		...
		$xml = <<<XML
		<media uri="$uri" sequence="" id="$this->mediaid" duration="$this->duration" direction="none" type="flash" name="$this->name" filename="$this->storedAs">
			<text></text>
			<template></template>
		</media>
XML;
	...
	public function AddForm()
	{
		...
		if ($regionid != "")
		{
			setSession('content','mediatype','flash');
...
END;
		...
		<div>
			<form id="file_upload" method="post" action="index.php?p=content&q=FileUpload" enctype="multipart/form-data" target="fileupload">
				...
				<table>
					<tr> 
						<td><label for="file">Flash File<span class="required">*</span></label></td>
						<td colspan="3">
		...
		<form class="dialog_file_form" method="post" action="index.php?p=module&mod=flash&q=AddMedia" enctype="multipart/form-data">
			...
			<table>
				<tr>
		    		<td><label for="name" title="The name of the flash. Leave this blank to use the file name">Name</label></td>
...
END;
		...
		<form class="dialog_file_form" method="post" action="index.php?p=module&mod=flash&q=EditMedia" enctype="multipart/form-data">
			<input type="hidden" name="MAX_FILE_SIZE" value="1048576000">
			<input type="hidden" name="layoutid" value="$layoutid">
			<input type="hidden" name="regionid" value="$regionid">
			...
			<table>
				<tr>
		    		<td><label for="name" title="The name of the Flash file. Leave this blank to use the file name">Name</label></td>
		    		<td><input id="name" name="name" type="text" value="$this->name"></td>
				</tr>
				<tr>
		    		<td><label for="duration" title="The duration in seconds this flash file should be displayed (may be overridden on each layout)">Duration<span class="required">*</span></label></td>
...
FORM;
	...
	public function DeleteForm()
	{
		...
		//we can delete
		$form = <<<END
		<form class="dialog_form" method="post" action="index.php?p=module&mod=flash&q=DeleteMedia">
			<input type="hidden" name="mediaid" value="$mediaid">
			<input type="hidden" name="lkid" value="$lkid">
			<input type="hidden" name="layoutid" value="$layoutid">
			<input type="hidden" name="regionid" value="$regionid">
...
END;
	...
	public function AddMedia()
	{
		...
		if ($ext != "swf")
		{
			$this->message .= "Flash must be SWF files";
			return false;
		}
		...
		$SQL .= "VALUES ('$name', 'flash', '$duration', '$fileName', $permissionid, " . $_SESSION['userid'] . ", 0)";
	...
	public function EditMedia()
	{
		...
		if ($fileRevision)
		{
			...
			$SQL .= "VALUES ('$name', 'flash', '$duration', '$fileName', $permissionid, " . $_SESSION['userid'] . ", 0, $mediaid )";
