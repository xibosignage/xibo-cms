...
FORM;
		...
		$this->response->dialogTitle 	= 'Add New Flash';
...
END;
		...
		<div>
			<form id="file_upload" method="post" action="index.php?p=content&q=FileUpload" enctype="multipart/form-data" target="fileupload">
				...
				<table>
					<tr> 
						<td><label for="file">New Flash File<span class="required">*</span></label></td>
						<td colspan="3">
							<input type="file" name="media_file" onchange="fileFormSubmit();this.form.submit();" />
						</td>
					</tr>
...
FORM;
		...
		$this->response->html 			= $form;
		$this->response->dialogTitle 	= 'Edit Flash';
		$this->response->dialogSize 	= true;
		$this->response->dialogWidth 	= '450px';
		$this->response->dialogHeight 	= '280px';
...
END;
		...
		$this->response->html 			= $form;
		$this->response->dialogTitle 	= 'Delete Flash';
		$this->response->dialogSize 	= true;
		$this->response->dialogWidth 	= '450px';
		$this->response->dialogHeight 	= '280px';
	...
	public function AddMedia()
	{
		...
		if ($tmpName == '')
		{
			$this->response->SetError('Cannot save Flash details. <br/> You must have picked a file.');
			$this->response->keepOpen = true;
			return $this->response;
		}
		...
		if ($ext != "swf")
		{
			$this->response->SetError('Only SWF files are accepted - Are you sure this is an flash?');
			$this->response->keepOpen = true;
			return $this->response;
		}
		...
		$SQL = sprintf("UPDATE media SET storedAs = '%s' WHERE mediaid = %d", $storedAs, $mediaid);
	...
	public function EditMedia()
	{
		...
		{
			$this->response->loadFormUri = "index.php?p=content&q=displayForms&sp=add";			
			$this->response->message = 'Edited the Flash.';
