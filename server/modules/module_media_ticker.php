...
class media_ticker {
	...
	private function SetRegionInformation($layoutid, $regionid, $mediaid)
	{
		...
		$this->template	 = (string) $mediaNode->template;
		...
		$this->copyright = (string) $mediaNode->copyright;
	...
	{	
		$xml = <<<XML
		<media uri="$this->uri" sequence="" id="$this->mediaid" duration="$this->duration" direction="$this->direction" type="ticker" name="$this->uri" filename="" lkid="">
			<text></text>
			<template>
				<![CDATA[
					$this->template
...
XML;
	...
	public function AddForm()
	{
		...
		$form = <<<FORM
		<form class="dialog_text_form" method="post" action="index.php?p=module&mod=ticker&q=AddMedia">
...
FORM;
	...
	public function EditForm()
	{
		...
		$direction_list = listcontent("none|None,left|Left,right|Right,up|Up,down|Down,single|Single", "direction", $this->direction);
		
		//Output the form
		$form = <<<FORM
		<form class="dialog_text_form" method="post" action="index.php?p=module&mod=ticker&q=EditMedia">
			<input type="hidden" name="layoutid" value="$layoutid">
			...
			<table>
				<tr>
					<td><label for="uri" title="The Link for the RSS feed">Link<span class="required">*</span></label></td>
					<td><input id="uri" name="uri" value="$this->uri" type="text"></td>
					<td><label for="copyright" title="Copyright information to display as the last item in this feed.">Copyright</label></td>
					<td><input id="copyright" name="copyright" type="text" value="$this->copyright" /></td>
				</td>
				<tr>
		    		<td><label for="direction" title="The Direction this text should move, if any">Direction<span class="required">*</span></label></td>
		    		<td>$direction_list</td>
				...
				<tr>
					<td colspan="4">
						<textarea id="ta_text" name="ta_text">$this->template</textarea>
					</td>
				</tr>
				<tr>
					<td colspan="4"><input type="checkbox" id="termsOfService" name="termsOfService" checked="checked"><label for="termsOfService">I certify I have the right to publish this media and that this media does not violate the terms of service stated in the <a href="http://www.xibo.org.uk/manual/index.php?p=content/license/termsofservice">manual</a>.</label></td>
...
FORM;
	...
	public function DeleteForm()
	{
		...
		//we can delete
		$form = <<<END
		<form class="dialog_form" method="post" action="index.php?p=module&mod=ticker&q=DeleteMedia">
			<input type="hidden" name="mediaid" value="$mediaid">
			<input type="hidden" name="layoutid" value="$layoutid">
...
END;
	...
	public function AddMedia()
	{
		...
		if ($uri == "" || $uri == "http://")
		{
			$this->message .= "Please enter a RSS Link";
			return false;
		}
