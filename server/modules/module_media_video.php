...
class media_video {
 	...
 	public function AsXml()
	{
		...
		$xml = <<<XML
		<media uri="$uri" sequence="" id="$this->mediaid" duration="$this->duration" direction="none" type="video" name="$this->name" filename="$this->storedAs">
			<text></text>
			<template></template>
		</media>
XML;
...
END;
		...
		<form class="dialog_file_form" method="post" action="index.php?p=module&mod=video&q=AddMedia" enctype="multipart/form-data">
			...
			<table width="100%">
				...
				<tr>
					<td><label for="duration" title="The duration in seconds this image should be displayed">Duration</label></td>
...
END;
		...
		<form class="dialog_file_form" method="post" action="index.php?p=module&mod=video&q=EditMedia" enctype="multipart/form-data">
			<input type="hidden" name="MAX_FILE_SIZE" value="1048576000">
			<input type="hidden" name="layoutid" value="$layoutid">
			<input type="hidden" name="regionid" value="$regionid">
			...
			<table width="100%">
				<tr>
		    		<td><label for="name" title="The name of the video file. Leave this blank to use the file name">Name</label></td>
		    		<td><input id="name" name="name" type="text" value="$this->name"></td>
				</tr>
				<tr>
					<td><label for="duration" title="The duration in seconds this video should be displayed">Duration</label></td>
		    		<td><input id="duration" name="duration" type="text" value="$this->duration"></td>
					<td><label for="permissionid">Sharing<span class="required">*</span></label></td>
					<td>
					$shared_list
				...
				<tr>
					<td></td>
					<td colspan="4">This form accepts: <span class="required">wmv, mpeg and mpg</span> files up to a maximum size of <span class="required">$this->maxFileSize</span>.</td>
				</tr>
				<tr>
					<td></td>
					<td colspan="4"><input type="checkbox" id="termsOfService" name="termsOfService" checked="checked"><label for="termsOfService">I certify I have the right to publish this media and that this media does not violate the terms of service stated in the <a href="http://www.xibo.org.uk/manual/index.php?p=content/license/termsofservice">manual</a>.</label></td>
...
FORM;
	...
	public function DeleteForm()
	{
		...
		//we can delete
		$form = <<<END
		<form class="dialog_form" method="post" action="index.php?p=module&mod=video&q=DeleteMedia">
			<input type="hidden" name="mediaid" value="$mediaid">
			<input type="hidden" name="lkid" value="$lkid">
			<input type="hidden" name="layoutid" value="$layoutid">
			<input type="hidden" name="regionid" value="$regionid">
...
END;
	...
	public function AddMedia()
	{
		...
		{
			$this->message .= "Video files supported: wmv, mpeg, mpg [this is $ext]";
			return false;
		}
		...
		$SQL .= "VALUES ('$name', 'video', '$duration', '$fileName', $permissionid, " . $_SESSION['userid'] . ", 0)";
	...
	public function EditMedia()
	{
		...
		else
		{
			...
			if ($ext != "wmv" || $ext != "mpeg" || $ext != "mpg")
			{
				$this->message .= "Video files supported: wmv, mpeg, mpg";
				return false;
			}
		}
		...
		if ($fileRevision)
		{
			...
			$SQL .= "VALUES ('$name', 'video', '$duration', '$fileName', $permissionid, " . $_SESSION['userid'] . ", 0, $mediaid )";
