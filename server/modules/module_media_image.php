...
END;
		...
		<div>
			<form id="file_upload" method="post" action="index.php?p=content&q=FileUpload" enctype="multipart/form-data" target="fileupload">
				...
				<table>
					<tr> 
						<td><label for="file">Image File<span class="required">*</span></label></td>
						<td colspan="3">
		...
		<form class="dialog_file_form" method="post" action="index.php?p=module&mod=image&q=AddMedia" enctype="multipart/form-data">
...
END;
		...
		<form class="dialog_file_form" method="post" action="index.php?p=module&mod=image&q=EditMedia" enctype="multipart/form-data">
			<input type="hidden" name="MAX_FILE_SIZE" value="1048576000">
			<input type="hidden" name="layoutid" value="$layoutid">
			<input type="hidden" name="regionid" value="$regionid">
...
FORM;
	...
	public function DeleteForm()
	{
		...
		//we can delete
		$form = <<<END
		<form class="dialog_form" method="post" action="index.php?p=module&mod=image&q=DeleteMedia">
			<input type="hidden" name="mediaid" value="$mediaid">
			<input type="hidden" name="lkid" value="$lkid">
			<input type="hidden" name="layoutid" value="$layoutid">
			<input type="hidden" name="regionid" value="$regionid">
...
END;
	...
	public function AddMedia()
	{
		...
		{
			$this->message .= "Only images are accepted - Are you sure this is an image?";
			return false;
		}
		if ($name == "")
		{
		...
		$SQL .= "VALUES ('$name', 'image', '$duration', '$fileName', $permissionid, " . $_SESSION['userid'] . ", 0)";
	...
	public function EditMedia()
	{
		...
		if ($db->num_rows($result) != 0)
		{
			$this->message = "Some media you own already has this name. Or this media has already been revised.";
			return false;
		}
		...
		if ($fileRevision)
		{
			...
			$SQL .= "VALUES ('$name', 'image', '$duration', '$fileName', $permissionid, " . $_SESSION['userid'] . ", 0, $mediaid )";
		...
		else
		{
			...
			if ($this->storedAs != "" && !file_exists($databaseDir."tn_".$this->storedAs)) 
			{
				//Create the thumbnail
				ResizeImage($databaseDir.$storedAs, $databaseDir."tn_".$storedAs, 80, 80);
			}
