...
(function(window, factory) {
  ...
  const Tour = (function() {
    ...
    function Tour(options) {
      ...
      this._options = $.extend(true, {
        ...
        backdropOptions: {
          ...
          animation: {
            // can be string of css class or
            // function signature: function(domElement, step) {}
        ...
        onEnd: function(tour) {
          const tourName = tour._options.name;
        ...
        onElementUnavailable: this.onElementUnavailable,
      ...
      if (typeof(this._options.sanitizeFunction) == 'function') {
        ...
        const whiteList = $.extend(true, {}, $.fn.popover.Constructor.Default.whiteList);
    ...
    Tour.prototype.start = function(fromStep) {
      ...
      const promise = this._makePromise(this._options.onStart != null ? this._options.onStart(this) : void 0);
    ...
    Tour.prototype.onElementUnavailable = function(tour, step) {
      const currentStep = tour.getStep(step);
    ...
    Tour.prototype.hideStep = function() {
      ...
      hideStepHelper = (function(_this) {
        return function(e) {
          ...
          const tmpModalOriginalElement = _this._getStepFlag(_this.getCurrentStepIndex(), 'elementModalOriginal');
    ...
    Tour.prototype.showStep = function(i) {
      ...
      const revalidateDelayElement = function(step) {
      ...
      showStepHelper = (function(_this) {
        return function(e) {
          if (_this._isOrphan(step)) {
            const $delayElement = revalidateDelayElement(step);
            const hasDelayElement = (step.delayOnElement && step.delayOnElement.delayElement != 'element');
      ...
      if ({}
        ...
        if (step.delayOnElement) {
          ...
          let $delayElement = revalidateDelayElement(step);

          const delayElementLog = $delayElement.length > 0 ? $delayElement[0].tagName : step.delayOnElement.delayElement;
        ...
        this.funcModalHelper = function(_this, $_modalObject) {
          return function() {
            ...
            if (typeof(step.onModalHidden) == 'function') {
              ...
              const rslt = step.onModalHidden(_this, i);
    ...
    Tour.prototype._showNextStep = function(skipOrphanOpt) {
      let promise;
      let showNextStepHelper;
      let step;

      const skipOrphan = skipOrphanOpt || false;
      ...
      if ($(document).find('.popover.tour-' + this._options.name + '.tour-' + this._options.name + '-' + this.getCurrentStepIndex()).length == 0) {
        ...
        if (step.reflex && !isOrphan) {
          ...
          if (step.reflexOnly) {
            ...
            const $objNext = $(step.template).find('[data-role="next"]').clone();

            if ($objNext.length) {
              // Get the DOM code for the object
              const strNext = $objNext[0].outerHTML;

              $objNext.hide();

              // Get the DOM code for the hidden object
              const strHidden = $objNext[0].outerHTML;
    ...
    Tour.prototype._createOverlayElements = function() {
      ...
      const $backdrop = $('<div class="tour-backdrop" id="' + DOMID_BACKDROP.substr(1) + '"></div>');
      const $highlight = $('<div class="tour-highlight" id="' + DOMID_HIGHLIGHT.substr(1) + '" style="width:0px;height:0px;top:0px;left:0px;"></div>');
    ...
    Tour.prototype._createStepSubset = function(step) {
      const _this = this;
      const _stepElement = $(step.element);
    ...
    Tour.prototype._showHighlightOverlay = function(step) {
      ...
      const $elemTmp = $('.tour-highlight-element');
      if ($elemTmp.length > 0) {
      ...
      const $modalCheck = $(step.element).parents('.modal:first');
