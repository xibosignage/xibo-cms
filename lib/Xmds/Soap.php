...
class Soap
{
    ...
    protected function phoneHome()
    {
        if ($this->getConfig()->getSetting('PHONE_HOME') == 1) {
            ...
            // If it's been > 1 day since last PHONE_HOME then
            if ($this->getConfig()->getSetting('PHONE_HOME_DATE') < (time() - (60 * 60 * 24))) {
                ...
                try {
                    ...
                    $this->getConfig()->changeSetting('PHONE_HOME_DATE', time());

                    // Patch
                    // Enhanced usage stats patch from 4.3, with modifications for v2.
                    // ---------------------------------------------------------------
                    $data = [
                        'id' => $key,
                    ...
                    // Connectors
                    $data['isSspConnector'] = 0;
                    $data['isDashboardConnector'] = 0;
                    ...
                    ', [
                        'fromDt' => Carbon::now()->startOfMonth()->format('U'),
                        'toDt' => Carbon::now()->endOfMonth()->format('U'),
                    ]);
