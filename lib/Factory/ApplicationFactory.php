...
class ApplicationFactory extends BaseFactory implements ClientRepositoryInterface
{
    ...
    public function setApplicationApproved($clientId, $userId, $approvedDate, $approvedIp)
    {
        ...
        $this->getStore()->insert('
            INSERT INTO `lkClientUser` (`clientId`, `userId`, `approvedDate`, `approvedIp`)
    ...
    {
        $results = $this->getStore()->select('SELECT clientId, userId FROM `lkClientUser` WHERE clientId = :clientId AND userId = :userId', [
            'userId' => $userId,
            'clientId' => $clientId
        ]);
    ...
    {
        return $this->getStore()->select('SELECT oauth_clients.name, oauth_clients.id, approvedDate, approvedIp FROM `lkClientUser` INNER JOIN oauth_clients on lkClientUser.clientId = oauth_clients.id WHERE `lkClientUser`.userId = :userId', [
            'userId' => $userId
        ]);
    }
    ...
    {
        $this->getStore()->update('DELETE FROM `lkClientUser` WHERE clientId = :clientId AND userId = :userId', [
            'userId' => $userId,
            'clientId' => $clientId
        ]);
    }
