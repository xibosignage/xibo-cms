...
export function useFilteredTabs(parentRoutePath: string): TabNavItem[] {
  const { user } = useUserContext();

  const parentRoute = APP_ROUTES.find((route) => route.path === parentRoutePath);

  if (!parentRoute || !parentRoute.subLinks || !user) {
    return [];
  }
