...
import { mockCampaign } from '../campaignTestUtils';
...
describe('useCampaignActions - handleConfirmMove', () => {
  ...
  it('moves all items and shows success notification', async () => {
    ...
    const itemsToMove = [
      { ...mockCampaign, campaignId: 1 },
      { ...mockCampaign, campaignId: 2 },
    ];
  ...
  it('shows error notification when all moves fail', async () => {
    ...
    await act(async () => {
      await result.current.handleConfirmMove([{ ...mockCampaign, campaignId: 1 }], 10);
    });
