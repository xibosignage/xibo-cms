...
describe('usePlaylistActions - handleConfirmMove', () => {
  ...
  it('moves all items and shows success notification', async () => {
    ...
    const itemsToMove = [
      { ...mockPlaylist, playlistId: 1 },
      { ...mockPlaylist, playlistId: 2 },
    ];
  ...
  it('shows error notification when all moves fail', async () => {
    ...
    await act(async () => {
      await result.current.handleConfirmMove([{ ...mockPlaylist, playlistId: 1 }], 10);
    });
