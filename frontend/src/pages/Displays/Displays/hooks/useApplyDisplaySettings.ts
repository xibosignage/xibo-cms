...
export function useApplyDisplaySettings() {
  ...
  const apply = async (
    ...
    try {
      ...
      // Group membership is a separate endpoint. Do it after the edit succeeded, so a bad group
      // cannot cost the operator the name and folder they chose.
      if (settings.displayGroupId) {
        await assignDisplayGroups(display.displayId, [settings.displayGroupId]);
      }
