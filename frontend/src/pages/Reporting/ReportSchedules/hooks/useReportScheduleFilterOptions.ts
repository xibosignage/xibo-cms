...
import { getBaseFilterKeys } from '../ReportSchedulesConfig';
...
export function useReportScheduleFilterOptions(t: TFunction) {
  ...
  const [reportTypeOptions, setReportTypeOptions] = useState<FilterOption[]>([]);
  const [reportDescriptionMap, setReportDescriptionMap] = useState<Record<string, string>>({});
    ...
    fetchReports()
      .then((data) => {
        ...
        Object.values(data)
          .flat()
          .forEach((r) => {
            opts.push({ label: r.description, value: r.name });
            map[r.name] = r.description;
          });
        setReportTypeOptions(opts);
        setReportDescriptionMap(map);
      })
      .catch(() => {});
    return () => {
      ignore = true;
  ...
  const filterOptions = getBaseFilterKeys(t).map((item) => {
    ...
    if (item.name === 'reportName') {
      return { ...item, options: reportTypeOptions };
    }

    return item;
  });

  return { filterOptions, reportDescriptionMap, isLoading: isLoadingOwners };
}
