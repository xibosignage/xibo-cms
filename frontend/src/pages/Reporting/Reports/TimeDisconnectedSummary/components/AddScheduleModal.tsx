...
interface AddScheduleModalProps {
  isOpen: boolean;
  currentFilter: TimeDisconnectedSummaryFilter;
  onClose: () => void;
  onSuccess: () => void;
}
...
export default function AddScheduleModal({
  ...
  const displayGroupOptions = useDisplayGroupOptions(isOpen);
  ...
  return (
    <ReportScheduleModalShell
      isOpen={isOpen}
      title={t('Schedule Time Connected Summary Report')}
      onClose={onClose}
      onSuccess={onSuccess}
      buildPayload={(draft) => ({
        name: draft.name,
        reportName: 'timedisconnectedsummary',
        filter: draft.filter,
        groupByFilter: '',
        displayGroupIds: [],
        displayId,
