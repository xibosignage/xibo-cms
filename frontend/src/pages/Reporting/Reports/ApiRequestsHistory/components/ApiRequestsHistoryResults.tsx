...
import type { ApiRequestsLogType } from '../ApiRequestsHistoryConfig';
...
import type { ApiRequestsRow } from '@/services/apiRequestsApi';

interface ApiRequestsHistoryResultsProps {
  rows: ApiRequestsRow[];
  logType: ApiRequestsLogType;
  isFetching: boolean;
  isError: boolean;
  onRefresh: () => void;
...
function textCol<K extends keyof ApiRequestsRow>(
  key: K,
  header: string,
  size: number,
  truncate = false,
): ColumnDef<ApiRequestsRow> {
  return {
    accessorKey: key,
    header,
    size,
...
function getColumns(
  t: TFunction,
  logType: ApiRequestsLogType,
  onViewDetails: (row: ApiRequestsRow) => void,
): ColumnDef<ApiRequestsRow>[] {
  if (logType === 'debug') {
    return [
...
export default function ApiRequestsHistoryResults({
  rows,
  logType,
  isFetching,
  isError,
  ...
  sorting,
  onSortingChange,
}: ApiRequestsHistoryResultsProps) {
  const { t } = useTranslation();
  const [detailsRow, setDetailsRow] = useState<ApiRequestsRow | null>(null);
      ...
      <div className="flex-1 min-h-0 relative flex flex-col">
        ...
        {showError ? (
          ...
          <DataTable
            ...
            loading={isFetching}
            hideToolbar
            getRowId={(row, index) => `${row.logId ?? row.applicationId}-${index}`}
          />
        )}
      </div>
