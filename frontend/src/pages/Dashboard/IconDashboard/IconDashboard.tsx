...
export default function IconDashboard() {
  ...
  const cardClasses =
    'flex items-center gap-4 rounded-lg border border-gray-200 bg-slate-50 py-4 px-8 text-sm font-medium col-span-2 cursor-pointer';
    ...
    <div className="mx-auto w-full p-5">
      <div className="grid grid-cols-4 gap-4">
        {visibleItems.map((item) => {
          ...
          return (
            <Link key={item.path} to={item.path} className={cardClasses}>
              {content}
            </Link>
          );
        })}
