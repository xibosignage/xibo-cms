...
import { getTaskColumns, getBulkActions, INITIAL_FILTER_STATE } from './TasksConfig';
...
export default function Tasks() {
  ...
  const {
    ...
    setFilterInputs,
    isHydrated,
  } = useTableState<TaskFilterInput>('task_page', {
  ...
  const [selectionCache, setSelectionCache] = useState<Record<string, Task>>({});
  ...
  const taskList = queryData?.rows ?? [];
  const pageCount = Math.ceil((queryData?.totalCount || 0) / pagination.pageSize);
  const error = isError && queryError instanceof Error ? queryError.message : '';
  const isConfigLocked = taskList[0]?.isConfigLocked ?? false;

  const getRowId = (row: Task) => String(row.taskId);
  ...
  const getAllSelectedItems = (): Task[] => {
    ...
    <section className="flex h-full w-full min-h-0 relative outline-none overflow-hidden">
      <div className="flex-1 flex flex-col min-h-0 min-w-0 px-5 pb-5">
        <div className="flex flex-row justify-between py-4 items-center gap-4">
          <TabNav activeTab="Tasks" navigation={adminTabs} />
