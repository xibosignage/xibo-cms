...
jQuery.fn.extend({
  xiboElementsRender: function(options, items) {
    ...
    const resetRenderElements = function($contentDiv) {
      // Remove item classes
      $contentDiv.find(options.marqueeInlineSelector)
        .removeClass(options.marqueeInlineSelector.replace('.', ''));

      // Remove animation items
      $contentDiv.find('.elements-render-anim-item').remove();

      // Show and reset the hidden elements
      $contentDiv.find('.elements-render-hidden-element')
        .removeClass('elements-render-hidden-element').show();
    };
    ...
    this.each(function(_key, element) {
      ...
      if (options.effect != 'none') {
        for (let i = 0; i < items.length; i++) {
          ...
          $newItem.addClass('text-render-item text-render-anim-item').
              appendTo(appendTo);
      ...
      if (options.effect == 'none') {
        ...
        let slides =
          (options.itemsPerPage > 1) ?
            '.elements-render-page' :
            options.marqueeInlineSelector;
        ...
        $(
          options.marqueeInlineSelector +
            ':not(.elements-render-hidden-element)',
          $contentDiv,
        ).each(function(_idx, _el) {
          if (!$(_el).hasClass('elements-render-hidden-element')) {
            $(_el).css({
              display: 'inline',
              'padding-left': '10px',
            });
      ...
      if (marquee) {
        ...
        const scroller = $('<div/>').addClass('scroll');
      ...
      if (paddingRight > 0 || paddingBottom > 0) {
        ...
        $(options.marqueeInlineSelector).css(
          'padding',
        ...
        $(`.text-render-page ${options.marqueeInlineSelector}`)
          .css('padding', 0);
        $(`#content ${options.marqueeInlineSelector}`).css('padding', 0);
      }
