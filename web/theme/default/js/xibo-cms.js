...
function XiboFormRender(sourceObj, data) {
    ...
    if (typeof sourceObj === "string" || sourceObj instanceof String) {
        ...
        formUrl = sourceObj.prop("href");
        // Remove the link from the source object if exists
        // Some browsers (Safari/IE) do not actually remove native props so removeAttr/removeProp doesn't work
        sourceObj.prop('href', '');
    }  
    
    // To fix the error generated by the double click on button
    if( ! formUrl ){
        //We want XiboFormRender to be chainable, however this is an 'invalid' click, return a rejected promise
    ...
    // Click handling is handled by the event functions higher up
    // Return a deferred Promise, this makes this function chainable
    return jqXHR;
}
